...
public class CastleCreator : MonoBehaviour
{
	...
	int floorAmount = 0; // for naiming mainly
	...
	[SerializeField] private GameObject m_culling;
	...
	public void CreateCastle(CastleData data)
	{
		...
		// Calculate navMesh position and size
		// height position is here just a temp value

		data.NavMeshPos = new Vector3(data.coordX, 0, data.coordZ);
		data.NavMeshSize = new Vector3(1000, 1000, 1000); // just test values
		...
		*/
		...
		// Test: update CulingGroups
		//Debug.Log("Set up Culling");
		m_culling.GetComponent<cullingTest2>().Setup();
	...
	*/
	...
	public void InstantiateCastleTiles()
	{
		...
		aParent.name = "Castle_" + m_data.mapX + "_" + m_data.mapZ + "_floorAmount_" + floorAmount.ToString();
